...
def assign_user(board_id):
    ...
    if user in board.users:
        flash('Этот пользователь уже добавлен.', 'warning')
        return redirect(url_for('board', board_id=board.id))

    board.users.append(user)
    db.session.commit()

    flash(f'Пользователь {user.email} добавлен на доску!', 'success')
    return redirect(url_for('board', board_id=board.id))
...
def handle_join_chat(data):
    ...
    join_room(card_id)
    print(f'User joined room for card {card_id}')

@socketio.on('send_message')
def handle_send_message(data):
    message = data['message']
    card_id = data['card_id']
    emit('receive_message', {'user': 'User', 'message': message, 'created_at': datetime.now().strftime('%Y-%m-%d %H:%M')}, room=card_id)
